...
fn select_popup(combobox: &str, multiselectable: &str, options: &str) -> String {
    format!(
        r#"<div class="popup select-popup" data-placement="bottom" data-flip="" data-shift="" data-sync="width" data-auto-size="vertical" data-auto-size-padding="10">{combobox}<div class="popup-body" popover="manual"><div class="listbox" role="listbox" tabindex="-1" aria-multiselectable="{multiselectable}" hidden>{options}</div></div></div>"#
    )
}
