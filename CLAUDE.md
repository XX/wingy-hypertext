...
4. Embeds shared sub-structs as fields tagged `#[as_ref] #[as_mut]`: `CommonAttrs` (id/classes/styles), `Link` (href/target/...), `Action` (data-action/data-action-args). These `AsRef`/`AsMut` impls are what make the blanket trait impls (below) apply to the component.
