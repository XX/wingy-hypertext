...
Заметил баг: кнопка очистки выбора в компоненте `Select` (при `with_clear == true`) немного расширяет высоту поля input компонента, внутри которого она находится. Высота "прыгает", сокращается, если кнопка исчезает после удаления всего выбранного. В исходном `wa-select` такого нет.

---

● Симптом знакомый: у <button class="clear-button"> в light DOM продолжают действовать глобальные стили нативных кнопок из native.css, где всем кнопкам задана height: var(--wa-form-control-height) — у WA этого
  нет из-за shadow DOM. Проверю гипотезу:

● Баг исправлен. Причина — снова разница light DOM vs shadow DOM:

  Механизм. Глобальные стили нативных кнопок в native.css задают всем <button> высоту var(--wa-form-control-height) — полную высоту поля ввода. У WA кнопка очистки живёт в shadow DOM и этих стилей не видит, а
  наша <button class="clear-button"> — видит, и select.css переопределял padding/border/background, но не height. Комбобокс — border-box с рамкой и min-height: var(--wa-form-control-height): его контентная
  область на две толщины рамки ниже, чем --wa-form-control-height, поэтому кнопка полной высоты распирала его на ~2px. Когда после очистки кнопка скрывалась (hidden), высота «спрыгивала» назад — ровно описанный
  эффект.

  Исправление (webassets/style/components/select.css): кнопке очистки заданы width: 1em; height: 1em — по размеру её иконки, с комментарием о причине. Это тот же приём, которым уже была защищена кнопка-крестик
  в тегах (.tag .tag-remove в tag.css имеет height/width: 1em — поэтому теги комбобокс не распирали). Визуально ничего не меняется: в WA кнопка тоже фактически 1em (иконка при padding: 0), центрирование в
  комбобоксе идёт через align-items: center.

  Статика пересобрана, фикс в target/web проверен. Попутная заметка: у этой кнопки остаётся ещё пара безвредных наследований от нативных кнопочных стилей (например, transform при :active из недавней
  синхронизации тем) — на геометрию они не влияют, но если захотите полной изоляции внутренних кнопок компонентов от native.css, это можно вынести в отдельную задачу.
